...
- Previews now work at all. Walrus aggregators serve blobs with no
  `Content-Type`, so every blob was categorized as binary and fell through to
  the "no inline preview" placeholder. Types are now detected from the content
  itself (magic bytes, plus UTF-8/JSON/markup sniffing).
- The three sample blob IDs were placeholders the aggregator rejected with
  HTTP 400. They are real mainnet blobs now, stored for 53 epochs.

### Changed

- Project renamed to **Morsu** (morsu.xyz) — from "Walrus Blob Explorer".
- The aggregator now defaults to Mainnet instead of Testnet.
